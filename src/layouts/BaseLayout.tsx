...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    if (!isTouchEnabled()) {
      ...
      const handleClick = () => {
        cursor.classList.remove('animate-slow-spin')
        cursor.classList.add('animate-cursor-click');

        setTimeout(() => {
          cursor.classList.remove('animate-cursor-click');
          cursor.classList.add('animate-slow-spin')
        }, 500)
      };
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div
        id="cursor"
        className="mix-blend-difference hidden fixed rounded-full pointer-events-none z-9999 w-28 h-28 animate-slow-spin ease-out duration-200"
      >
